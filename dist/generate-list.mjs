 ...
 */
function getMarkdownFilesWithFrontmatter(dirPath) {
    ...
    // 根据日期排序
    markdownFiles.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
 ...
 */
function generateListContent(markdownFiles) {
    let content = `# 目录\n\n`;
 ...
 * @param dirPath - 目标目录路径
 */
function writeListFile(dirPath) {
 ...
 */
function processDirectoriesRecursively(rootDir, blacklist = []) {
